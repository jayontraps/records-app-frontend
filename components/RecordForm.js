...
const RecordForm = props => {
  ...
  const [classification, setClassification] = useState(
    isUpdate ? update.classification : ''
  )
  ...
  async function uploadFile(file) {
    ...
    const res = await fetch(
      'https://api.cloudinary.com/v1_1/dtceo0fjk/image/upload',
      {
        method: 'POST',
        body: data
      }
  ...
  return (
    ...
    >
      ...
      <div className="field field__date">
        <div className="field-status">
          <h3>Date</h3>
          {showRequiredMsg && !date && (
            <span className="required">Start date is required</span>
          )}
        </div>
        <SingleDatePicker
      ...
      {classification && classification.label === 'Birds' && (
        <div className="field field__breeding">
          <ExpandPanel heading="Breeding Code">
            <BreedingOptions
              currentBreedingCode={breedingCode}
              changeHandler={e => setBreedingCode(e.target.value)}
            />
          </ExpandPanel>
        </div>
      )}
